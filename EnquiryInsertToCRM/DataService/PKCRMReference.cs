...
using EnquiryInsertToCRM.PKServiceReference;
...
{
    public static class PKCRMReference
    {
        ...
        public static string UDFFieldsName = "MYOB IsInactive,MYOB Sales Person,MYOB Identifiers," + FinancialYearCRMFolder + "MYOB Last Sale Date,MYOB Products bought,Unused Fields\\MYOB Items bought," +
            ...
            "Unused Fields\\MYOB Overdue Balance,Unused Fields\\Last Payment Date";
        public static string GetToken()
        {
            ...
            JObject authRequest = new JObject(
                new JProperty("Database", ConfigurationManager.AppSettings["PKMaximizerDatabase"]),
                new JProperty("UID", ConfigurationManager.AppSettings["PKMaximizerUID"]),
                new JProperty("Password", ConfigurationManager.AppSettings["PKMaximizerPassword"])
              );

            //instantiate the WCF service client ("MaximizerWebDataService" is the service namespace)
            using (PKServiceReference.DataClient client = new PKServiceReference.DataClient())
            {
        ...
        public static string UpdateCompanyNameOrMyObIdInCRM(string companyname, string myobUID)
        {
            ...
            try
            {
                if (CommonMethod.IsExpiredTokenForPk_BasedOnCookies())
                {
                ...
                if (!string.IsNullOrEmpty(model.Key))
                {
                    ...
                    PKServiceReference.DataClient client = new PKServiceReference.DataClient();
                                ...
                                if (!string.IsNullOrEmpty(Key))
                                {
                                    ...
                                    if (custSaleInvObj != null && custSaleInvObj.Items != null)
                                    {
                                        ...
                                        foreach (var item in custSaleInvObj.Items.Where(s => s.Status == "Open").OrderByDescending(s => s.Date))
                                        {
                                            ...
                                            JToken jt1 = CommonMethod.MakeAccountRightAPICall_SingleItemReturn(item.Uri, CommonMethod.GetAccessToken_Pk(), client_id, cftoken);
                                            if (jt1 != null)
                                            {
                                            ...
                                            JToken jt2 = CommonMethod.MakeAccountRightAPICall_SingleItemReturn(strUrl, CommonMethod.GetAccessToken_Pk(), client_id, cftoken);
                                            if (jt2 != null)
                                            {
                                    ...
                                    var lstCRList = CommonMethod.MakeAccountRightAPICall(strUrl, CommonMethod.GetAccessToken_Pk(), client_id, cftoken);
        ...
        public static string CreateCompany(CustomerInfo objCustomerInfo)
        {
            ...
            if (objCustomerInfo.Addresses != null && objCustomerInfo.Addresses.Count > 0)
            {
                ...
                {
                    lstJProperty.Add(new JProperty("Email", objCustomerInfo.CustomField3.Value));
                    divNoteText.AppendLine("<div style='margin-bottom:5px;'><b>Email:</b> " + objCustomerInfo.CustomField3.Value + "</div>");
                }
                if (!string.IsNullOrEmpty(objCustomerInfo.Uid))
                {
        ...
        public static string getMyOBUid_UDFKey()
        {
            List<AbEntryFieldInfo> udfFieldList = new List<AbEntryFieldInfo>();
            string udfkey = "";
            udfFieldList = PKCRMReference.GetFieldList();
            if (udfFieldList != null && udfFieldList.Count > 0)
            {
                var objUdf = udfFieldList.Select(s => s).Where(s => s.Name.ToLower().Trim() == "myob uid".Trim()).FirstOrDefault();
                if (objUdf != null)
